...
class BloomFilter {
    ...
    public boolean contains(String s) {
        ...
        // Looping through all of the possible hash functins
        for (int i = 0; i < noHashes; i++) {
            // Computing the hash value for the string
            long hashValue = hashCode(s, i);
            // Convert the hash value into a valid bit index within the BitSet range
            int bitIndex = (int) hashValue & hashMask;
            // Checking if the corresponding bit is in the set
            if (!data.get(bitIndex)) { // If not
                return false; // String not in the set
            }
        } // Otherwise
