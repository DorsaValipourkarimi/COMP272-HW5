...
/* 
	public void put(K key, V value) {
		...
		for (int i = 0; i < CAPACITY; i++) {
			...
			// If same key but different value, continue; otherwise skip
			if (table[pos1].getBucKey().equals(key) &&
				!table[pos1].getValue().equals(value)) {
				// continue — allowed duplicate key with different value
			}
	
			// Evict and continue to alternate position
			Bucket<K, V> evicted = table[pos1];
			table[pos1] = newBucket;
			newBucket = evicted;
			key = newBucket.getBucKey();
			value = newBucket.getValue();
			...
			// If same key but different value, continue; otherwise skip
			if (table[pos2].getBucKey().equals(key) &&
				!table[pos2].getValue().equals(value)) {
				// continue — allowed duplicate key with different value
			}
	
			// Evict again and loop
			evicted = table[pos2];
			table[pos2] = newBucket;
			newBucket = evicted;
			key = newBucket.getBucKey();
		...
		// After CAPACITY attempts, assume a cycle and rehash
		rehash();
